...
- **Model discovery.** Copilot's config flow calls `GET /models` to populate its model picker. The proxy answers this itself: an operation policy lists the Foundry account's deployments via the Azure Resource Manager control-plane API (`GET .../Microsoft.CognitiveServices/accounts/{account}/deployments`, authenticated with the APIM managed identity) and returns them in OpenAI list-models format, so the picker shows only the models actually deployed on your Foundry instance — not the full Foundry catalog. Each deployment name is the value clients pass as `model`.
...
- The `/models` operation is served entirely within APIM: it lists the Foundry account's deployments through the Azure Resource Manager control-plane API with the managed identity and returns them in OpenAI list-models format, so Copilot's model picker reflects the real deployments rather than the whole catalog. The managed identity's existing `Cognitive Services OpenAI User` role already grants the required `Microsoft.CognitiveServices/*/read`. Tune the lookup with `foundryDeploymentsApiVersion` (default `2024-10-01`).
