...
For model discovery, Copilot first calls `GET /byok/models`. APIM handles this without hitting the chat backend: the `/models` operation policy lists the Foundry account's deployments (`GET /openai/deployments`, managed-identity auth) and returns them in OpenAI list-models format, so the picker shows only the deployments that actually exist on the account.
